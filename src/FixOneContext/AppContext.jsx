...
const ContextProvider = ({children}) => {
  ...
  const mostRecentCountry = countriesList[countriesList.length - 1 > 0 ? countriesList.length - 1 :  0];
  const waitTime = useMemo(() => 300 , []);
  return(
...
const CreateCountry = (props) => {
  const {countriesList, setCountriesList} = useContext(ContextTest);
